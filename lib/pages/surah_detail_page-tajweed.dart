...
class _SurahDetailPageState extends State<SurahDetailPage> {
  ...
  Future<void> fetchSurah() async {
    // Load Arabic + translations from API
    final res = await http.get(Uri.parse(
        "https://maranaw.com/api/surah_json.php?surah=${widget.surahNumber}"));
    final data = json.decode(res.body);
    final apiAyahs = List<Map<String, dynamic>>.from(data['data']['ayahs']);

    // Load Tajweed JSON from local assets
    final tajweedRaw = await rootBundle
        .loadString("assets/data/tajweed/${widget.surahNumber}.json");
    final tajweedData = json.decode(tajweedRaw);

    // Merge ayah by ayah
    for (var i = 0; i < apiAyahs.length; i++) {
      if (i < tajweedData["ayahs"].length) {
        apiAyahs[i]["text_tajweed_html"] =
            tajweedData["ayahs"][i]["text_tajweed_html"] ?? "";
        // 🔹 Debug print dito
        print("SURAH ${widget.surahNumber} AYAH ${apiAyahs[i]['ayah_number']}");
        print(apiAyahs[i]["text_tajweed_html"]);
      }
    }
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TajweedText(
                          htmlText: ayah['text_tajweed_html'] ?? (ayah['text_ar'] ?? ""),
                          fontSize: 26,
                          showColors: _showColors, // make sure you declared this in your state
